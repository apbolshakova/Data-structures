...
status_t deleteVer(version_t* verToDelete)
{
	...
	if (buf->parentPtr == verToDelete && copyChild() == FAIL) //если буфер - сын удал€емой версии, то его тоже перекинуть на родител€ verToDelete
	{
	...
	//освободить пам€ть verToDelete, но не трогать детей и операции
	return SUCCESS;
}
