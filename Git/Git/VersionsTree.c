...
status_t merge(int verNum)
{
	...
	{
		operation_t* opListForMerging = getOpListForMerging(bufDiff, verDiff); //из ver выт€нуть все операции, при добавлении которых в буфер произойдЄт merge
		if (!opListForMerging)
		{
			deletePath(&pathToBuf);
			deletePath(&pathToVer);
	...
	//сформировать diff дл€ буфера и дл€ версии
	//сопоставить коды у символов: -2 может быть только с нулЄм
	//если это ок - то сливаем версии вместе (добавл€ем в буфер операции из ver, индексы действи€ которых сдвинуты в зависимости от diff (исходный + сдвиги из buf + сдвиги из ver))
}