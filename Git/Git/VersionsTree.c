...
status_t deleteVer(version_t* verToDelete)
{
	...
	cleanupVersion(verToDelete); //освободить пам€ть verToDelete, но не трогать детей и 
...
status_t copyVerChildren(version_t* prevParent)
{
	if (!(prevParent->parentPtr)) //handle if it's root
	{
		...
		{
			//формируемый список операций - отмена всех операций нулевого ребЄнка + операции ребЄнка, присвоить созданный список как список операций ребЄнка, addChild
		}
	}
	else
	{
