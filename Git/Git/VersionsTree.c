...
status_t deleteVer(version_t* verToDelete)
{
	...
	if (buf->parentPtr == verToDelete && addChild(buf, verToDelete->parentPtr) == FAIL) //если буфер - сын удал€емой версии, то его тоже перекинуть на родител€ verToDelete
	{
	...
	cleanupVersion(verToDelete); //освободить пам€ть verToDelete, но не трогать детей и операции
	return SUCCESS;
}
...
status_t copyVerChildren(version_t* prevParent)
{
	...
	{
		//¬ыбрать новый корень
		//ѕрикрепить к нему остальных детей
	}
	else
	{
